...
pass_field = "password"
if "passwordHash" in schema:
    pass_field = "passwordHash"
elif "hash" in schema:
    pass_field = "hash"

# Check bcrypt package
has_bcryptjs = os.path.exists("node_modules/bcryptjs")
bcrypt_import = "import bcrypt from 'bcryptjs';" if has_bcryptjs else "import crypto from 'crypto';"

# Generate TypeScript seed code
ts_content = f"""
import {{ PrismaClient }} from '@prisma/client';
{bcrypt_import if has_bcryptjs else ''}

const prisma = new PrismaClient();

async function main() {{
  console.log('--- Seeding Database with New Organization Structure ---');
  
  const defaultPass = '{pass_field}';
  let passValue = '$2a$10$vI8aWBnW3fID.ZQ4/zo1G.q1l5QkO6qOefqj6lqMvK3MvMh7p7xey'; // Bcrypt for Dev@2026
...
with open("prisma/seed_clean.ts", "w") as f:
    f.write(ts_content)

print("Created prisma/seed_clean.ts successfully!")
