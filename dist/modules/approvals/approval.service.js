...
const processApproval = async (approvalId, actorId, action, remarks) => {
    ...
    if (actor.role !== "OWNER") {
        if (!actor.locationId ||
            ...
            throw new Error("You can only process expenses from your assigned location");
        }
    }
    // ----------------------------------------------------------
    // REJECTION
        ...
        return prisma_js_1.prisma.$transaction(async (tx) => {
            ...
            await tx.notification.createMany({
                data: users.map((user) => ({
                    ...
                    message: `Expense of ₹${approval.expense.amount} has been rejected by ${actor.name}.` +
    ...
    return prisma_js_1.prisma.$transaction(async (tx) => {
        ...
        if (actor.role === "OWNER") {
            ...
            if (!cashier) {
                throw new Error("No active cashier found for this expense location");
            }
            ...
            await tx.notification.create({
                data: {
                    ...
                    message: `Expense of ₹${currentApproval.expense.amount} ` +
