...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (rawRole.includes('OWNER') || rawRole.includes('ADMIN')) normalizedRole = 'OWNER';
    else if (rawRole.includes('MANAGER') || rawRole.includes('MGR')) normalizedRole = 'MANAGER';
    else if (rawRole.includes('CASH') || rawRole.includes('FINANCE')) normalizedRole = 'CASHIER';

    const token = jwt.sign(
      { userId: user.id, employeeId: user.employeeId, role: normalizedRole, locationId: user.locationId },
      JWT_SECRET,
      { expiresIn: '30d' }
    );
